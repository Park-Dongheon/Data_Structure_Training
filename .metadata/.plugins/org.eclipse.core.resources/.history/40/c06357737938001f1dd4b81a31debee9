...
//		return key.hashCode() % size;
	...
	//--- 키값이 key인 요소를 검색(참/거짓을 반환) ---//
	public int search2(SimpleObject5 st, Comparator<? super SimpleObject5> c) {
	...
	public SimpleObject5 search(SimpleObject5 st, Comparator<? super SimpleObject5> c) {
		...
		while (selectN != null) {						// 선택 노드가 null 일 때까지 반복
			if (c.compare(st, selectN.getData()) == 0)	// 선택 노드의 key(data)값과 검색할 key 값일 일치할 경우
	...
	public int delete(SimpleObject5 st, Comparator<? super SimpleObject5> c) {
		int hash = hashValue(st);							// 삭제할 데이터의 해시값
		Node2 selectN = table[hash];						// 선택 노드
		Node2 prevN = null;									// 이전 노드, 선택 노드를 따라가는 노드, 선택 노드 바로 앞을 가리킴
		
		while (selectN != null) {							// 선택 노드가 null 일 때까지 반복
			if (c.compare(st, selectN.getData()) == 0) {	// 삭제할 키 값을 찾은 경우
				if (prevN == null) {						// 이전 노드가 null 인 경우, 해시 값이 충돌이 발생하지 않은 경우
					table[hash] = selectN.next;				// 해시 테이블의 값을 선택 노드의 링크로 설정
				}
				else {										// 해시 값의 충돌이 발생하여 오프 해시법(Chaining)로 LinkedList 인 경우
	...
	public static void main(String[] args) {
		...
		do {
			...
			case Search:							// 검색
				...
				result = hash.search2(data, SimpleObject5.NO_ORDER);
				SimpleObject5 exit = hash.search(data, SimpleObject5.NO_ORDER);
				if (result == 1)
