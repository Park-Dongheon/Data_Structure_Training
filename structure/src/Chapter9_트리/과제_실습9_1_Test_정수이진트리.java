...
class Tree5 {
	...
	// 이진 트리를 레벨 순서로 출력, BFS(너비 우선 탐색)를 사용하여 구현
	void levelOrder() 									// level 별로 출력한다. level 이 증가하면 다음줄에 출력한다
	// 난이도: 최상급 구현
	{
		ObjectQueue5 q = new ObjectQueue5(20);			// 노드를 저장할 큐 생성 (노드들을 level 순으로 저장), 노드 큐
		Queue<Integer> que = new LinkedList<>();		// level 정보를 저장할 큐 생성, 레벨 큐
		int oldLevel = 0,  newLevel=0;					// 현재 level(oldLevel)과 새로운 level(newLevel) 변수 초기화
		TreeNode5 currentNode = root;					// 현재 노드를 트리의 root 로 설정
		
		
		if (currentNode == null) {						// 트리가 비어 있는 경우
			return;										// 아무것도 출력하지 않고 종료
		}
		
		q.enque(currentNode);							// root 노드를 노드 큐에 추가
		que.add(oldLevel + 1);							// root 노드의 level(1)을 레벨 큐에 추가
		
		while (!q.isEmpty()) {							// 노드 큐가 비어 있지 않을 동안 반복
			currentNode = (TreeNode5) q.deque();		// 큐에서 노드를 하나 꺼내 currentNode 에 저장
			oldLevel = newLevel;						// oldLevel을 현재의 newLevel로 업데이트
			newLevel = que.remove();					// 큐에서 level 정보를 꺼내 newLevel에 저장
			
			if (newLevel > oldLevel) {					// level 이 증가하면
				if (oldLevel != 0)						// 첫 level 이 아니면
					System.out.println();				// 줄 바꿈 (다음 레벨로 이동)
			}
			
			System.out.print(currentNode.data + " ");	// 현재 노드의 데이터를 출력
			
			if (currentNode.LeftChild != null) {		// 현재 노드의 왼쪽 자식이 존재하면
				q.enque(currentNode.LeftChild);			// 왼쪽 자식을 노드 큐에 추가
				que.add(newLevel + 1);					// 왼쪽 자식의 level 정보를 레벨 큐에 추가
			}
			
			if (currentNode.RightChild != null) {		// 현재 노드의 오른쪽 자식이 존재하면
				q.enque(currentNode.RightChild);		// 오른쪽 자식을 노드 큐에 추가
				que.add(newLevel + 1);					// 오른쪽 자식의 level 정보를 레벨 큐에 추가
			}
		}
		System.out.println();							// 마지막 줄 바꿈 (마지막 level 출력 후)
	}
